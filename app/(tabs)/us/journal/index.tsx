...
function journalDateMs(row: EntryRow): number {
  if (row.entryDate) {
    const parsed = startOfDay(parseISO(row.entryDate)).getTime();
    if (Number.isFinite(parsed)) return parsed;
  }
  return row.createdAt;
}

function compareJournalRows(a: EntryRow, b: EntryRow): number {
  return journalDateMs(b) - journalDateMs(a) || b.createdAt - a.createdAt;
}

function journalDisplayDateLabel(row: EntryRow): string {
  const timestamp = journalDateMs(row);
  if (!timestamp) return '';
  const date = new Date(timestamp);
  if (isToday(date)) return row.createdAt ? format(new Date(row.createdAt), 'h:mm a') : 'Today';
  if (isYesterday(date)) return 'Yesterday';
  return format(date, 'MMM d');
}

function journalShortDateLabel(row: EntryRow): string {
  const timestamp = journalDateMs(row);
  return timestamp ? format(new Date(timestamp), 'MMM d') : '';
}
...
function JournalScreenInner() {
  ...
  const authorMeta = (id: string): { name: string; color: string } => {
    if (!id || id === userId) return { name: myName || 'You', color: C.accent };
  ...
  }, [allEntries, userId, partnerId, partnerName, myName, members, C.accent, C.accent2, C.accent3]);

  const stats = useMemo(() => {
    const total = rows.length;
    const monthMs = startOfMonth(new Date()).getTime();
    const thisMonth = rows.filter((r) => journalDateMs(r) >= monthMs).length;
  ...
  const featured = useMemo(() => {
    return rows.slice().sort(compareJournalRows)[0] ?? null;
  }, [rows]);
  ...
  const buckets = useMemo<Bucket<EntryRow>[]>(() => {
    ...
    return order
      ...
      .map((k) => ({
        label: k,
        dotColor: dotMap[k],
        rows: groups[k].slice().sort(compareJournalRows),
      }));
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      <Stack.Screen
        options={{
          ...
          headerRight: () => (
            <PressScale
              onPress={() => router.push('/sheets/journal-entry' as any)}
              hitSlop={12}
              haptic="impact"
              pressedScale={0.96}
              style={{ padding: 4 }}
                ...
                <>
                  ...
                  <View style={styles.featuredFoot}>
                    ...
                    <Text style={[Typography.mono, { color: C.ink3, fontSize: 10 }]}>
                      {journalShortDateLabel(featured)}
                    </Text>
                    {featured.isPrivate ? (
                      <View
                        style={[
        ...
        <View style={styles.listWrap}>
          {buckets.length === 0 ? (
            <ActionEmptyState
              ...
              accent={C.journal}
              onAction={() => router.push('/sheets/journal-entry' as any)}
            />
          ) : (
            <BucketedList
              buckets={buckets}
              ...
              renderRow={(e) => {
                ...
                const dateLabel = journalDisplayDateLabel(e);
                return (
                  <SwipeableRow
                    ...
                    onDelete={e.isMine ? () => remove(e.id) : undefined}
                  >
                    <PressScale
                      onPress={() =>
                        router.push(`/journal/${e.id}` as any)
                      }
                      haptic="impact"
                      pressedScale={0.96}
                      style={styles.row}
