...
function TimetableDetailInner() {
  ...
  const deleteItem = (item: TimetableItem) => remove(String(item.id));
...
function GridBody({
  ...
  onDeleteItem: (item: TimetableItem) => void;
}) {
  const { C } = useTheme();
  return (
    <View style={{ paddingHorizontal: 18 }}>
      ...
      {dayItems.length === 0 ? (
        <ActionEmptyState
          icon="calendar"
          title="Open day"
          body={`Nothing scheduled on ${DAYS_FULL[selectedDay]}.`}
          accent={C.accent2}
        />
      ) : (
        <View style={{ gap: 10 }}>
          {dayItems.map((item) => (
            <SwipeableRow
              ...
              deleteMessage="This timetable block will be removed."
            >
              <PressScale
