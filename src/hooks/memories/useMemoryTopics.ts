 ...
 */
export function useMemoryTopics(spaceId: string | null | undefined, userId: string | null | undefined) {
  const query = useMemo(() => {
    if (!spaceId) return null;
    return {
      memories: {
        $: { where: { 'space.id': spaceId }, limit: 500 },
        author: {},
      },
    };
  }, [spaceId]);
  ...
  const memories: any[] = (data as any)?.memories ?? [];
